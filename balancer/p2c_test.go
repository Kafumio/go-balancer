...
// TestP2C_AddHost .
func TestP2C_AddHost(t *testing.T) {
...
// TestP2C_RemoveHost .
func TestP2C_RemoveHost(t *testing.T) {
	rnd := rand.New(rand.NewSource(time.Now().UnixNano()))
	h1 := &host{"http://127.0.0.1:1011", 0}
	h2 := &host{"http://127.0.0.1:1012", 0}
	cases := []struct {
...
func TestP2C_Balance(t *testing.T) {
	...
	t.Run(c.name, func(t *testing.T) {
		c.lb.IncConn("http://127.0.0.1:1013")
		c.lb.IncConn("http://127.0.0.1:1013")
		c.lb.IncConn("http://127.0.0.1:1")
		c.lb.DoneConn("http://127.0.0.1:1")
		c.lb.DoneConn("http://127.0.0.1:1013")
