...
func NewHTTPProxy(targetHosts []string, algorithm string) (*HTTPProxy, error) {
	...
	balancer, err := balancer.Build(algorithm, hosts)
	if err != nil {
		return nil, err
	}
	return &HTTPProxy{
...
func (h *HTTPProxy) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	host, err := h.balancer.Balance(GetIP(r)) // 根据 IP 选择合适的主机
	if err != nil {
		w.WriteHeader(http.StatusBadGateway)
		_, _ = w.Write([]byte(fmt.Sprintf("balance error: %s", err.Error())))
	}
	h.balancer.IncConn(host)
	defer h.balancer.DoneConn(host)
